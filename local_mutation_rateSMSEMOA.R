...
CC_SMSEMOA <- function(population=NULL,
  ...
  while(budget_left>0){
    ...
    {  
      ...
      parentCD <- cd[parentIndex]
      mainParent <- sample(1:2,1)
      effectiveCD <- parentCD[mainParent]
      
      parentMD <- md[parentIndex]
      effectiveMD <- parentMD[mainParent]
      ...
      offspring <- nsga2R::boundedPolyMutation(parent_chromosome = offspring,
                                               lowerBounds = lbound[groupMember],
                                               upperBounds =  ubound[groupMember],
                                               mprob=pm,mum = effectiveMD)
      # evaluate objective
      this_offspring <- offspring[1,,drop=FALSE]
      
      #one parent
      closestCV <- population[parentIndex[mainParent],]
      ...
      if(step_adapt){
        if(newPointSurvives){
          ...
          for(whichParent in 1:2){
            popId <- parentIndex[whichParent]
            cd[popId] <- max(c((cd[popId] +1)/alpha-1,0))
            md[popId] <- max(c((md[popId] +1)/alpha-1,15))
          }
          ...
          for(whichParent in 1:2){
            popId <- parentIndex[whichParent]
            cd[popId] <- min(c((cd[popId] +1)*alpha-1,50))
            md[popId] <- min(c((md[popId] +1)*alpha-1,30))
          }
          last100success <- last100success[-1]
    ...
    # if(nEval%%500==0){
    #   print("plot")
    #   print(GetHypervolume(t(populationObjective),
    #                        reference = refPoint,
    #                        method = hypervolumeMethod))
    #   plot(0,0,xlim=c(0,6),ylim=c(0,10),type="l")
    #   
    #   points(populationObjective)
