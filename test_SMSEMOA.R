...
# source('local_mutation_rateSMSEMOA.R')  
set.seed(100)
...
for (testID in c(2:7))
{
  ...
  for(i in 1:10){
    ...
    gs <- gs[testIDinUse]
    if(!base){
      abc <- CC_SMSEMOA( population = NULL, nVar = nVar, populationSize  = 100,
                         fun = transposedFun, mainfunc=func, k=k, nObj =2,growPop=T,
                         maxPopulationSize=100,nIterInner=nIterInner,
                         control=ctrl,group_selection=gs)
    }else{
      population <- MaOEA::InitializePopulationLHS(100,1000,lbound,ubound)
      for(i in 1:budget){
    ...
    save(list=ls(),file=paste0("global_stepsize_adaptation/",nIterInner,'iter_ID',testID,'_instance',i,'.Rdata'))
  }
}
