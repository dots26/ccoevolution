...
#' @export
MOSACC <- function(contextVectors=NULL,objs=NULL,popSize=50,fun,...,
  ...
  for(groupIndex in 1:nGroup){
    ...
    CMAES_control[[groupIndex]] <- list(vectorized=vectorized,
                                        ...
                                        mu=floor(4+floor(3*log(groupSize))/2),lambda=(4+floor(3*log(groupSize)))*4,
  ...
  contextVectors <- contextVectors[,rnk[[1]][1],drop=F]
  objs <- objs[,rnk[[1]][1],drop=F]
  contextVectorCount <- 1
  # objs <- objs[,rnk[[1]]]
  ...
  while(budget_left>0){
    ...
    for (CVIndex in shuffle) {
      ...
      while(groupIndex<=length(group)){
        ...
        bestHV <- MaOEA::GetHypervolume(objs*scale,reference*scale)
        groupMember <- group[[groupIndex]]
        ...
        budget_left <- budget-nEval
        print(paste('budget:',budget_left))
        if(!is.null(best$value)){
          newCV <- contextVector
          newCV[groupMember] <- best$par
          newObj <- fun(newCV,...)
          newObj_set <- objs
          exceedRef <- any(newObj>reference)

          graphics.off()
          x <- (0:1000)/1000
          y <- (1-x^2)^0.5
          plot(x,y,xlim=c(0,2),ylim=c(0,2),type='l')
          points(objs[1,]/2,objs[2,]/4,col='red')
          points(newObj[1,]/2,newObj[2,]/4,col='blue')
          ...
          if(!add_point){
            ...
            if(!exceedRef)
              newHV <- MaOEA::GetHypervolume(newObj_set[,rnk[[1]]]*scale,reference = reference*scale)
            if(exceedRef)
              newHV <- MaOEA::GetHypervolume(newObj_set[,rnk[[1]]]*scale) # dynamic reference
          }else{
      ...
      if(budget_left>0){
        if(updated){
          ...
          newObj <- fun(contextVector,...)
          newObj_set <- objs
          if(add_point){
            newObj_set <- cbind(newObj_set,newObj)
            newCV <- cbind(newCV,contextVector)
          }else{
            newObj_set[,CVIndex] <- newObj
            newCV[,CVIndex] <- contextVector
          }
          rnk <- nsga2R::fastNonDominatedSorting(t(newObj_set))
          ...
          if(contextVectorCount==length(rnk[[1]])){
            if(add_point){
              ...
              if(removedPoint<=contextVectorCount){
                contextVectors[,removedPoint] <- contextVector
          ...
          print(paste('HV to 1.1*PF:',MaOEA::GetHypervolume(objs,reference=1.1*c(2,4))))
