#' @export
UX_SMSEMOA <- function(population=NULL,
  ...
  while(budget_left>0){
    ...
    while(nEval%/%growingPeriod>nTimesGrow){
      ...
      #                                     upperBounds =  ubound,
      #                                     cprob = pc,mu = cd)
      offspring <- uniformCrossover(parent_chromosome = population[parentIndex,],
                                    lowerBounds = lbound,
                                    upperBounds =  ubound,
                                    cprob = pc,nOffspring = 1)
    ...
    {  xx <- (0:1000)/1000*2
      ...
      #Crossover
      offspring <- uniformCrossover(parent_chromosome = population[parentIndex,groupMember],
...
uniformCrossover <- function(parent_chromosome, lowerBounds, upperBounds, cprob,nOffspring)
{
  popSize = nrow(parent_chromosome)
  nVar = ncol(parent_chromosome)
  child <- parent_chromosome
  p <- 1
  for (i in 1:nOffspring) {
    if (runif(1) < cprob) {
      parentIndex <- sample(x = 1:popSize,replace = F,size = 2)
      parent1 <- runif(nVar)<0.5
      child[i,which(parent1)] <- parent_chromosome[1,which(parent1)]
      child[i,which(!parent1)] <- parent_chromosome[2,which(!parent1)]
    }
  }
  return(child)
}